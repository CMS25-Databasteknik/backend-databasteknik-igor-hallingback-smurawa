...
public class InstructorRoleService(IInstructorRoleRepository repository) : IInstructorRoleService
{
    ...
    public async Task<InstructorRoleResult> CreateInstructorRoleAsync(CreateInstructorRoleInput input, CancellationToken cancellationToken = default)
    {
        ...
        {
            return new InstructorRoleResult { Success = false, StatusCode = 500, Message = $"An error occurred while creating the instructor role: {ex.Message}" };
        }
    }
    ...
    public async Task<InstructorRoleListResult> GetAllInstructorRolesAsync(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return new InstructorRoleListResult { Success = true, StatusCode = 200, Result = roles, Message = roles.Any() ? "Instructor roles retrieved successfully." : "No instructor roles found." };
        }
        catch (Exception ex)
        {
            return new InstructorRoleListResult { Success = false, StatusCode = 500, Message = $"An error occurred while retrieving instructor roles: {ex.Message}" };
        }
    }
    ...
    public async Task<InstructorRoleResult> GetInstructorRoleByIdAsync(int id, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (role == null)
            {
                return new InstructorRoleResult { Success = false, StatusCode = 404, Message = $"Instructor role with ID '{id}' not found." };
            }

            return new InstructorRoleResult { Success = true, StatusCode = 200, Result = role, Message = "Instructor role retrieved successfully." };
        }
        catch (Exception ex)
        {
            return new InstructorRoleResult { Success = false, StatusCode = 500, Message = $"An error occurred while retrieving the instructor role: {ex.Message}" };
        }
    }
    ...
    public async Task<InstructorRoleResult> UpdateInstructorRoleAsync(UpdateInstructorRoleInput input, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (updated == null)
            {
                return new InstructorRoleResult { Success = false, StatusCode = 404, Message = $"Instructor role with ID '{input.Id}' not found." };
            }
        ...
        {
            return new InstructorRoleResult { Success = false, StatusCode = 500, Message = $"An error occurred while updating the instructor role: {ex.Message}" };
        }
    }
    ...
    public async Task<InstructorRoleDeleteResult> DeleteInstructorRoleAsync(int id, CancellationToken cancellationToken = default)
    {
        try
        {
            if (id < 1)
            {
                return new InstructorRoleDeleteResult { Success = false, StatusCode = 400, Message = "Id must be greater than zero.", Result = false };
            }

            var result = await _repository.DeleteInstructorRoleAsync(id, cancellationToken);
            if (!result)
            {
                return new InstructorRoleDeleteResult { Success = false, StatusCode = 404, Message = $"Instructor role with ID '{id}' not found.", Result = false };
            }

            return new InstructorRoleDeleteResult { Success = true, StatusCode = 200, Message = "Instructor role deleted successfully.", Result = true };
        }
        catch (Exception ex)
        {
            return new InstructorRoleDeleteResult { Success = false, StatusCode = 500, Message = $"An error occurred while deleting the instructor role: {ex.Message}", Result = false };
        }
    }
}
